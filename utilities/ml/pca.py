...
class DimensionalityReductionPCA:
    ...
    def __init__(self, settings):
        ...
        # if hasattr(settings, 'type'): #Parameters
        #     self._type = settings.type
        if 'type' in settings: #dict
            self._type = settings['type']
        else:
            self._type = 'pca'

        if 'n_components' in settings:
    ...
    def subspace_mesh(self, distance = None, n_points = None, boundaries = None):
        ...
        for i in range(0,self.n_components):
            ...
            point_a = self.invert_model(point_a_projected.reshape(1,-1)) #RFHH; add reshape
            point_b_projected = np.zeros(self.n_components)
            point_b_projected[i] = upper_bound[i]
            point_b = self.invert_model(point_b_projected.reshape(1,-1)) #RFHH; add reshape
    ...
    def save_stats(self, saveName, header=""): #RFHH
        with open(f'{saveName}.csv', 'w') as file:
            file.write(f"PCA Dimensionality Reduction Performance\n{header}\n")
            file.write(f"explained_variance,{self.model.explained_variance_}\n"
                       f"explained_variance_percentage,{self.model.explained_variance_ratio_}\n"
                       f"total_explained_variance_percentage,{np.sum(self.model.explained_variance_ratio_)}\n"
                       f"singular_values,{self.model.singular_values_}\n"
    ...
    def _run_pca(self, training_data, disp_result):
        ...
        if disp_result:
            print("Explained variance {}, sum {}".format(self.model.explained_variance_ratio_,sum(self.model.explained_variance_ratio_)))
